...
====================*/
struct vary_node ** second_pass() {
  struct vary_node ** frames = (struct vary_node **)calloc(sizeof(struct vary_node *), num_frames);
  int i,j=0;
  double incr = 0;
  while(i<lastop){
    if(op[i].opcode == VARY){
      if(!vary_node[0]){
        ...
        char *n = op[i].op.vary.p;
        for(j=0;j<frames;j++){
          struct vary_node * node = (struct vary_node *)calloc(sizeof(vary_node),1);
          node->name=n;
          if(j<=(int)op[i].op.vary.start_frame){
            node->value=op[i].op.vary.start_val;
          }else if(j>=(int)op[i].op.vary.end_frame){
            node->value=op[i].op.vary.end_val;
          }else{
